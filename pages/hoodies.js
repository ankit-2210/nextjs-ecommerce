            ...
            <section className="text-gray-600 body-font">
                <div className="container px-5 py-24 mx-auto">
                    <div className="flex flex-wrap -m-4 text-center justify-center">
                        {Object.keys(products).length == 0 && <h3 className='font-bold text-xl mt-10 mb-8'>Sorry! All the hoodies are currenty out of stock. New Stock coming soon. Stay Tunned !</h3>}
...
export async function getServerSideProps(context) {
    ...
    for (let item of products) {
        if (item.title in hoody) {
            if (!hoody[item.title].color.includes(item.color) && item.availableQty > 0) {
                hoody[item.title].color.push(item.color);
            }
            if (!hoody[item.title].size.includes(item.size) && item.availableQty > 0) {
                hoody[item.title].size.push(item.size);
            }
        }
        else {
            hoody[item.title] = JSON.parse(JSON.stringify(item));

            if (item.availableQty > 0) {
                hoody[item.title].color = [item.color];
                hoody[item.title].size = [item.size];
            }
        }
    }
    ...
    return {
        props: { products: JSON.parse(JSON.stringify(hoody)) }, // will be passed to the page component as props
    }
}
